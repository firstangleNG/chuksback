...
async function fetchInvoiceData(invoiceId) {
  ...
  async function printInvoice() {
    ...
    // const invoiceId = document.getElementById('invoice_no').value.trim();
    ...
    // Use the fetched data to populate the fields
    ...
    const email_address = invoiceData.customer_email || 'Not provided';
  
    // Get number of items taxed
    let number_of_taxed_items_counter = 0;
    rows.forEach(item => {
      if (parseInt(item.tax) > 0) {
    ...
    let tax_statement = '';
    number_of_taxed_items_counter > 0
      ? (tax_statement = `(20% tax inclusive on ${number_of_taxed_items_counter} item(s))`)
      : (tax_statement = '(0% tax inclusive)');
  
    // Generate the items HTML
    let itemsHtml = '';
    rows.forEach((item, index) => {
      const { service_name, description, quantity, price_per_unit, tax, total } = item;
      itemsHtml += `
        <tr>
           <td><strong>${index + 1}</strong></td>
           <td>${service_name}</strong><br>${description}</td>
           <td>${quantity}</td>
    ...
    const totalTax = invoiceData.total_tax;
    const finalAmount = invoiceData.final_amount;
   ...
   <div class="invoice-header-items">
    <span id="company-name">Sales Invoice<br>
      ...
      <div class="invoice-detail-single">
          ...
          <strong>Contact:</strong> info@computerhubuk.com | +44 00 0000 0000</p>
      </div>
      <div class="invoice-detail-single">
