...
const logoUrl = "/static/images/computer_hub_uk.png";
    ...
    printButton.addEventListener('click', () => {
        ...
        rows.forEach((row,index) => {
            ...
            const tax = row.querySelector('input[name="tax[]"]').value;
...
<body>
    <div class="invoice-header">
        <div class="invoice-header-items">
          <img src="https://firststore.codemeduck.com/static/images/computer_hub_new.jpeg" alt="Logo"><br>
        </div>
         <div class="invoice-header-items">
        ...
        <div class="totals">
          ...
          <p><strong>VAT line : 353 9011 12(0% Inclusive):${(totalTax).toLocaleString('en-GB',{
            style:"currency",
            currency:"GBP",
            minimumFractionDigits:2,
            maximumFractionDigits:2
    ...
    // <select  name="tax" class="tax default-fields">
...
document.getElementById("email-form").addEventListener("submit",async (e)=>{
    e.preventDefault();
    const invoice = localStorage.getItem('last_invoice_number');
    console.log('invoice',invoice)
    const customer_name = document.getElementById("name").value.trim();
    const email = document.getElementById("customer_email").value.trim();
    let csrfToken = document.querySelector("[name=csrfmiddlewaretoken]").value;

    const response = await fetch("/invoicing/email-invoice/", {
        method: "POST",
        headers: {
