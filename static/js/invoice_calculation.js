...
form.addEventListener('submit', async function(event) {
    ...
    const taxes = parseFloat(form.querySelector("select[name='tax']").value);
            ...
            responseData.data.forEach((item,index) => {
                ...
                row.innerHTML = `
                    ...
                    <td><select  name="tax" class="tax default-fields">
                       <option value="0" ${item.taxes == 0 ? 'selected' : ''}>Not Taxed</option>
                        <option value="20" ${item.taxes == 20 ? 'selected' : ''}>Taxed</option>
                     </select>
                    </td>
...
document.getElementById('update-invoice-btn').addEventListener('click', async () => {
    try {
        ...
        const items = Array.from(rows).map(row => {
            ...
            return {
                service_name: getVisibleInput(row,'input', 'service_name'),
                description: getVisibleInput(row,'input', 'description'),
                quantity: parseInt(getVisibleInput(row,'input', 'quantity'), 10),
                price_per_unit: parseFloat(getVisibleInput(row,'input', 'price_per_unit')),
                taxes: parseFloat(getVisibleInput(row,'select', 'tax')) || 0
            };
...
function getVisibleInput(row,type, name) {
    const input = row.querySelector(`${type}[name="${name}"]`);
